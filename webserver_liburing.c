...
bool check_copy_complete(struct request *req,int ret,int socket){
    struct iovec * iov = &req->iov[req->iovec_count-1];
    int file_size = iov->iov_len;
    printf("has write : %d kb , file_size : %d kb, remain size:%d kb\n",ret/1024,file_size/1024,(file_size - ret)/1024);
   
    if(ret <file_size){
        struct request *req_ = malloc(sizeof(*req_) + sizeof(struct iovec));
        req_->iovec_count = 1;
        req_->iov[0].iov_base = iov->iov_base + ret;
        req_->iov[0].iov_len = file_size - ret;
        req_->client_socket = socket;
        req_->file_buff = req->file_buff;
        sleep(1);
        
        
        add_write_request(req_);
        return false;
    }
    return true;
...
void send_headers(const char *path, off_t len, struct iovec *iov) {
    ...
    if(strcmp("zip",file_ext) == 0){
        strcpy(send_buffer,"Content-Type: application/gzip\r\n");
    }
    ...
    while (1) {
        ...
        switch (req->event_type) {
            ...
            case EVENT_TYPE_WRITE:{
                bool complete = check_copy_complete(req,cqe->res,req->client_socket);
